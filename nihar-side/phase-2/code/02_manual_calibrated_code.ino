...
const float POSTURE_THRESHOLD = 5.0; // degrees
const unsigned long BAD_TIME = 2000;  // 2 seconds
...
void loop()
{
    ...
    if (calibrated)
    {
        deviation = angleX - referenceAngle;
    }
    ...
    else if (abs(deviation) > POSTURE_THRESHOLD)
    {
        if (!badPosture)
        {
            badPosture = true;
...
// motor is not working properly and the 